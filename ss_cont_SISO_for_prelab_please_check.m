...
alpha_1 = 1.1722; 
alpha_2 = -7.9096;
% replace C in the following function with [C_SISO; 0 0 0 0]
[A, B, C, D] = create_ip_model(alpha_1, alpha_2);

% Fill in your choice of eigenvalues for A-BK and A-LC
all_ev = [-10 + 10i, -10 - 10i, -50, -30]; % need a vector here
all_ev_obs = 3*all_ev;  % e.g., all_ev_obs = 3*all_ev;

% Generate K matrix
K = place(A, B, all_ev);

% Generate L matrix
C_SISO = [1 -0.4 0 0]; % put the C_SISO here from your prelab Q1
L_SISO = place(A', C_SISO', all_ev_obs)';
L = [L_SISO -0.4.*L_SISO]; % should be 4x2

% save the controller

save_ss_control(alpha_1, alpha_2, K, L, 'my_ss_control_SISO'); 
